...
class WC_Product_Addons_Global_Group {
	...
	static public function get_group( $post ) {
		...
		$brand_term_ids = (array) wp_get_post_terms( $post->ID, array( 'sports' ), array( 'fields' => 'ids' ) );
		$price_term_ids = (array) wp_get_post_terms( $post->ID, array( 'price' ), array( 'fields' => 'ids' ) );
		...
		// MWB changes
		if ( 1 == get_post_meta( $post->ID, '_all_products_brands', true ) ) {
			$brand_categories = array();
		} else {
			$brand_categories = array();
			foreach ( $brand_term_ids as $term_id ) {
				$term = get_term_by( 'id', $term_id, 'sports' );
				if ( $term ) {
					$brand_categories[ $term_id ] = $term->name;
				}
			}
		}
		if ( 1 == get_post_meta( $post->ID, '_all_products_price', true ) ) {
			$price_categories = array();
		} else {
			$price_categories = array();
			foreach ( $price_term_ids as $term_id ) {
				$term = get_term_by( 'id', $term_id, 'price' );
				if ( $term ) {
					$price_categories[ $term_id ] = $term->name;
				}
			}
		}
		return array(
			...
			'restrict_to_brands'     => $brand_categories,
			'restrict_to_price'     => $price_categories,
			'fields'                 => $fields
		);
	}
