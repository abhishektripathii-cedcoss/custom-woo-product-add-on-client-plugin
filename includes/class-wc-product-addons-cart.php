...
class WC_Product_Addons_Cart {
	...
	public function __construct() {
		// Load cart data per page load.  MWB yha se cart pe aa rha hai price keval or checkout yhi se ja rha hai 
		add_filter( 'woocommerce_get_cart_item_from_session', array( $this, 'get_cart_item_from_session' ),1000, 2 );

		// Custom hook for pricing in cart page
		add_action( 'woocommerce_before_calculate_totals' , array( $this , 'wpswings_add_product_fee_in_cart' ) , 500 , 1 ) ;

		// Add item data to the cart.
		add_filter( 'woocommerce_add_cart_item_data', array( $this, 'add_cart_item_data' ), 110, 2 );
		...
		// Get item data to display.  Hook MWB It is showing the price cart cartmwb
	...
	public function wpswings_add_product_fee_in_cart($cart) {

		if ( !( is_plugin_active('deals-for-woocommerce/deals-for-woocommerce.php') ) ) {
			return;
		}

		if ( ! dfw_check_is_array( $cart->cart_contents ) ) {
			return ;
		}

		if ( 'yes' != get_option( 'dfw_product_deals_enable' ) ) {
			return ;
		}

		if ( ! dfw_allow_deals_for_guest() ) {
			return ;
		} else {
			if ( ! dfw_check_if_valid_user() ) {
				return ;
			}
		}

		foreach ( $cart->cart_contents as $value ) {
			$wpswings_warranty_price = 0;

			if( ! empty (  $value['addons'] )) {

				foreach( $value['addons'] as $pri=>$a ) {
					foreach( $a as $key=>$val ) {
						if( $key == 'price' ) {
							$wpswings_warranty_price = $wpswings_warranty_price + $val;
						}
					}
				}
			}
			$productid = empty( $value[ 'variation_id' ] ) ? $value[ 'product_id' ] : $value[ 'variation_id' ] ;
			...
			if ( dfw_check_is_array( $deal_ids ) ) {
				$deal_id = reset( $deal_ids ) ;
				$deal    = dfw_get_deal( $deal_id ) ;
				...
				if ( is_object( $deal ) ) {
					...
					if ( 'product' == $deal->get_type() ) {
						...
						if ( 'yes' == get_post_meta( $product_id , 'dfw_enable_deal' , true ) ) {

							$matched_data = DFW_Product_Handler::get_matched_rules( $product , $product_id , $deal_id , $deal->get_type() ) ;

							if ( dfw_check_is_array( $matched_data ) ) {

								if ( isset( $matched_data[ 'price' ] ) && isset( $matched_data[ 'rule_id' ] ) ) {

									$rule    = dfw_get_rule( $matched_data[ 'rule_id' ] ) ;
									$user_id = get_current_user_id() ;

									$user_limit = dfw_check_if_user_limit_is_reached( $rule , $user_id ) ;

									if ( ! $user_limit ) {
										$wpswings_set_custom_price = $wpswings_warranty_price + $matched_data[ 'price' ];
										$value[ 'data' ]->set_price( $wpswings_set_custom_price ) ;
									}
								}
							}
						}
	...
	public function add_cart_item_data( $cart_item_data, $product_id ) {
		...
		if ( empty( $cart_item_data2['addons'] ) ) {
			$cart_item_data2['addons'] = array();
		}
		...
		if ( is_array( $product_addons ) && ! empty( $product_addons ) ) {
			...
			foreach ( $product_addons as $addon ) {
				...
				if ( is_wp_error( $data ) ) {
					...
					$cart_item_data2['addons'] = array_merge( $cart_item_data2['addons'], apply_filters( 'woocommerce_product_addon_cart_item_data', $data, $addon, $product_id, $post_data ) );
				}
			}
		}
		$cart_item_data = array_merge( $cart_item_data, $cart_item_data2);
