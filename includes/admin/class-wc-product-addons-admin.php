...
class WC_Product_Addons_Admin {
	...
	public function __construct() {
		...
		// Adding custom script change 7
		add_action( 'admin_enqueue_scripts', array( $this, 'custom_js' ) );
	...
	// change 8
	/**
	 * Adds custom js for WooCommerce Quick View adding a new script.
	 */
	public function custom_js() {
		if ( is_admin() ) {
			// $suffix = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';
			// die();
			wp_enqueue_script( 'custom-js', WC_PRODUCT_ADDONS_PLUGIN_URL . '/assets/js/custom.js', array( 'jquery' ), WC_PRODUCT_ADDONS_VERSION, true );
		}
	}
	...
	public function global_addons_admin() {
		if ( ! empty( $_GET['add'] ) || ! empty( $_GET['edit'] ) ) {
			...
			if ( $_POST ) {
				...
				// change 6
				$objects_brand  = ! empty( $_POST['addon-objects-brand'] ) ? array_map( 'absint', $_POST['addon-objects-brand'] ) : array();
				$objects_price  = ! empty( $_POST['addon-objects-price'] ) ? array_map( 'absint', $_POST['addon-objects-price'] ) : array();
			...
			if ( ! empty( $_GET['edit'] ) ) {
				...
				// change 5
				$objects_brand  = (array) wp_get_post_terms( $global_addon->ID, apply_filters( 'woocommerce_product_addons_global_post_terms2', array( 'sports' ) ), array( 'fields' => 'ids' ) );
				$objects_price  = (array) wp_get_post_terms( $global_addon->ID, apply_filters( 'woocommerce_product_addons_global_post_terms2', array( 'price' ) ), array( 'fields' => 'ids' ) );
				...
				// change 4
				if ( get_post_meta( $global_addon->ID, '_all_products_brands', true ) == 1 ) {
					$objects_brand[] = 0;
				}
				// change 4
				if ( get_post_meta( $global_addon->ID, '_all_products_price', true ) == 1 ) {
					$objects_price[] = 0;
				}
				...
				$objects        = (array) wp_get_post_terms( $global_addon->ID, apply_filters( 'woocommerce_product_addons_global_post_terms', array( 'product_cat' ) ), array( 'fields' => 'ids' ) );
				// change 3
				$objects_brand  = (array) wp_get_post_terms( $global_addon->ID, apply_filters( 'woocommerce_product_addons_global_post_terms', array( 'sports' ) ), array( 'fields' => 'ids' ) );
				$objects_price  = (array) wp_get_post_terms( $global_addon->ID, apply_filters( 'woocommerce_product_addons_global_post_terms', array( 'price' ) ), array( 'fields' => 'ids' ) );
				...
				// first run here when create addon page opens  changes
				...
				$objects             = array( 0 );
				// change 1
				$objects_brand       = array( 0 );
				$objects_price       = array( 0 );
	...
	public function save_global_addons() {
		...
		if ( $edit_id ) {
			...
			// change 10
			wp_set_post_terms( $edit_id, $objects_brand, 'sports', false );
			wp_set_post_terms( $edit_id, $objects_price, 'price', false );
			...
			$edit_id = wp_insert_post( apply_filters( 'woocommerce_product_addons_global_insert_post_args', array(
				...
				'tax_input'     => array(
					'product_cat' => $objects,
					// change 11
					'sports'      => $objects_brand,
					'price'      => $objects_price,
				),
			), $reference, $objects, $objects_brand, $objects_price ) );
		}
		...
		// Custom  change 12
		if ( in_array( 0, $objects_brand ) ) {
			update_post_meta( $edit_id, '_all_products_brands', 1 );
		} else {
			update_post_meta( $edit_id, '_all_products_brands', 0 );
		}
		if ( in_array( 0, $objects_price ) ) {
			update_post_meta( $edit_id, '_all_products_price', 1 );
		} else {
			update_post_meta( $edit_id, '_all_products_price', 0 );
		}
