...
<div class="wrap woocommerce">
	...
	<form method="POST" action="">
		<table class="form-table global-addons-form meta-box-sortables">
			...
			<!-- adding custom code abhishek -->
			<tr>
				<th>
					<label for="addon-objects"><?php esc_html_e( 'Product brand', 'woocommerce-product-addons' ); ?></label>
				</th>
				<td>
					<select id="addon-objects-brand" name="addon-objects-brand[]" multiple="multiple" style="width:50%;" data-placeholder="<?php esc_attr_e( 'Choose categories&hellip;', 'woocommerce-product-addons' ); ?>" class=" wc-enhanced-select-brand wc-pao-enhanced-select">
						<option value="all" <?php selected( in_array( 0, $objects_brand ), true ); ?>><?php esc_html_e( 'All Products', 'woocommerce-product-addons' ); ?></option>
						<optgroup label="<?php esc_attr_e( 'Product brand', 'woocommerce-product-addons' ); ?>">
							<?php
							$terms2 = get_terms( 'sports', array( 'hide_empty' => 0 ) );

							foreach ( $terms2 as $term2 ) {
								echo '<option value="' . $term2->term_id . '" ' . selected( in_array( $term2->term_id, $objects_brand ), true, false ) . '>' . $term2->name . '</option>';
							}
							?>
						</optgroup>
						<?php do_action( 'woocommerce_product_addons_global_edit_objects', $objects_brand ); ?>
					</select>
					<p class="description"><?php esc_html_e( 'Select which brands this add-on should apply to. Create add-ons for a single product when editing that product.', 'woocommerce-product-addons' ); ?></p>
				</td>
			</tr>
			<tr>
				<th>
					<label for="addon-objects"><?php esc_html_e( 'Product Price', 'woocommerce-product-addons' ); ?></label>
				</th>
				<td>
					<select id="addon-objects-price" name="addon-objects-price[]" multiple="multiple" style="width:50%;" data-placeholder="<?php esc_attr_e( 'Choose price&hellip;', 'woocommerce-product-addons' ); ?>" class=" wc-enhanced-select-price wc-pao-enhanced-select">
						<option value="all" <?php selected( in_array( 0, $objects_price ), true ); ?>><?php esc_html_e( 'All Products', 'woocommerce-product-addons' ); ?></option>
						<optgroup label="<?php esc_attr_e( 'Product Price', 'woocommerce-product-addons' ); ?>">
							<?php
							$terms2 = get_terms( 'price', array( 'hide_empty' => 0 ) );

							foreach ( $terms2 as $term2 ) {
								echo '<option value="' . $term2->term_id . '" ' . selected( in_array( $term2->term_id, $objects_price ), true, false ) . '>' . $term2->name . '</option>';
							}
							?>
						</optgroup>
						<?php do_action( 'woocommerce_product_addons_global_edit_objects', $objects_price ); ?>
					</select>
					<p class="description"><?php esc_html_e( 'Select which price this add-on should apply to. Create add-ons for a single product when editing that product.', 'woocommerce-product-addons' ); ?></p>
				</td>
			</tr>
			<!-- adding custom code abhishek -->
...
<script type="text/javascript">
	jQuery( function( $ ) {
		...
		// custom code by abhishek
		$( '.wc-enhanced-select-brand' ).on( 'select2:select', function( e ) {
			var selectedID = e.params.data.id,
				values     = $( '.wc-enhanced-select-brand' ).val(),
				all        = 'all',
				allIndex   = values.indexOf( all );
			...
			$( '.wc-enhanced-select-brand' ).val( values ).trigger( 'change.select2' );
		} );
		$( '.wc-enhanced-select-price' ).on( 'select2:select', function( e ) {
			var selectedID = e.params.data.id,
				values     = $( '.wc-enhanced-select-price' ).val(),
				all        = 'all',
				allIndex   = values.indexOf( all );
			...
			$( '.wc-enhanced-select-price' ).val( values ).trigger( 'change.select2' );
		} );
	} );
</script>
