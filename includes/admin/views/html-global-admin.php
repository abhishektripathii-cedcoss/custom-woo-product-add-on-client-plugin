	...
	<table id="global-addons-table" class="wp-list-table widefat" cellspacing="0">
		<thead>
			<tr>
				...
				<th><?php esc_html_e( 'Product Brand', 'woocommerce-product-addons' ); ?></th>
				<th><?php esc_html_e( 'Product Price Range', 'woocommerce-product-addons' ); ?></th>
			...
			if ( $global_addons ) {
				foreach ( $global_addons as $global_addon ) {
					...
					<tr>
						...
						<td>
						<?php
						$all_products_brands           = '1' === get_post_meta( $global_addon['id'], '_all_products_brands', true ) ? true : false;
						$restrict_to_brand_categories = $global_addon['restrict_to_brands'];

						if ( $all_products_brands ) {
							esc_html_e( 'All Products', 'woocommerce-product-addons' );
						} elseif ( 0 === count( $restrict_to_brand_categories ) ) {
							esc_html_e( 'No Products Assigned', 'woocommerce-product-addons' );
						} else {
							$objects_brands    = array_keys( $restrict_to_brand_categories );
							$term_names_brands = array_values( $restrict_to_brand_categories );
							$term_names_brands = apply_filters( 'woocommerce_product_addons_global_display_term_names', $term_names_brands, $objects_brands );
							echo wp_kses_post( implode( ', ', $term_names_brands ) );
						}
						?>
						</td>
						<td>
						<?php
						$all_products_price           = '1' === get_post_meta( $global_addon['id'], '_all_products_price', true ) ? true : false;
						$restrict_to_price_categories = $global_addon['restrict_to_price'];

						if ( $all_products_price ) {
							esc_html_e( 'All Products', 'woocommerce-product-addons' );
						} elseif ( 0 === count( $restrict_to_price_categories ) ) {
							esc_html_e( 'No Products Assigned', 'woocommerce-product-addons' );
						} else {
							$objects_price    = array_keys( $restrict_to_price_categories );
							$term_names_price = array_values( $restrict_to_price_categories );
							$term_names_price = apply_filters( 'woocommerce_product_addons_global_display_term_names', $term_names_price, $objects_price );
							echo wp_kses_post( implode( ', ', $term_names_price ) );
						}
						?>
						</td>
