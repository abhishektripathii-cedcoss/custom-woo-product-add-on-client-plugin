...
class WC_Product_Addons_Helper {
		...
		if ( false === $cache_value || $last_modified !== $cache_value['last_modified'] ) {
			$args          = array(
				...
				'meta_query'          => array(
					...
					array(
						'key'   => '_all_products_brands',
					 	'value' => '1',
					),
					array(
						'key'   => '_all_products_price',
					 	'value' => '1',
					),
				),
			);
	...
	protected static function get_product_term_addons( $product_id ) {
		...
		if ( false === $cache_value || $last_modified !== $cache_value['last_modified'] ) {
			...
			$product_terms_2 = apply_filters( 'get_product_addons_product_terms2', wc_get_object_terms( $product_id, 'sports', 'term_id' ), $product_id );
			$product_terms_3 = apply_filters( 'get_product_addons_product_terms3', wc_get_object_terms( $product_id, 'price', 'term_id' ), $product_id );
			// change 14

			// if ( $product_terms || $product_terms_2 ) {
			if ( $product_terms || $product_terms_2 || $product_terms_3 ) {
				$args          = apply_filters(
					...
					array(
						...
						'tax_query'           => array(
							...
							array(
								'taxonomy'         => 'sports',
								'field'            => 'id',
								'terms'            => $product_terms_2,
								'include_children' => false,
							),
							array(
								'taxonomy'         => 'price',
								'field'            => 'id',
								'terms'            => $product_terms_3,
								'include_children' => false,
							),
						),
					),
	...
	public static function get_product_addons( $post_id, $prefix = false, $inc_parent = true, $inc_global = true ) {
		...
		if ( '1' !== $exclude && $inc_global ) {
			...
			$check =  self::get_global_product_addons();
			$check2 = self::get_product_term_addons( $product->get_id() );
