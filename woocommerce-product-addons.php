<?php
/**
 * Plugin Name: Custom WooCommerce Product Add-ons
...
add_filter( 'cron_schedules', 'wpswings_add_cron_interval' );
function wpswings_add_cron_interval( $schedules ) { 
...
add_action( 'init', 'create_price_taxonomy', 5 );

function create_price_taxonomy() {
	register_taxonomy(
		'price',
		'product',
		array(
...
// Code for adding the taxonomy to product.
add_action( 'my_task_hook', 'custom_update_price_cat');
/**
 * Callback function for adding custom taxonomy to products.
 *
 * @return void
 */
function custom_update_price_cat() {
	$wpswings_all_ids = get_posts( array(
		'post_type' => 'product',
		'numberposts' => -1,
		'post_status' => 'publish',
		'fields' => 'ids',
	...
	foreach ( $tax_terms as $term_single ) {
		$wpswings_price_range_string = $term_single->slug;
		$wpswings_term_id            = $term_single->term_id;
		$wpswings_price_range_array  = explode( '-', $wpswings_price_range_string );
		$wpswings_min_range          = (int) ( $wpswings_price_range_array[0] );
		$wpswings_max_range          = (int) ( $wpswings_price_range_array[1] );
		foreach ( $wpswings_all_ids as $id ) {
			$wpswings_regular_price = (int) get_post_meta( $id, '_regular_price', true );
			if ( ( $wpswings_min_range <= $wpswings_regular_price ) && ( $wpswings_regular_price <= $wpswings_max_range ) ) {
				wp_set_post_terms( $id, $wpswings_term_id, $taxonomy );
			}
		}
	}
}
...
function woocommerce_product_addons_init() {
	...
	if ( ! class_exists( 'WC_Product_Addons' ) ) :
		define( 'WC_PRODUCT_ADDONS_VERSION', '5.0.0' ); // WRCS: DEFINED_VERSION.
		...
		class WC_Product_Addons {
			...
			public function __construct() {
				...
				// custom addon for price category  update
			}
			...
			public function init_post_types() {
				...
				register_taxonomy_for_object_type( 'sports', 'global_product_addon' );
				register_taxonomy_for_object_type( 'price', 'global_product_addon' );
			}
